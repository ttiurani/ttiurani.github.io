...
* *Share alike*. All source code is licenced with AGPLv3 and all text content with CC BY-SA 4.0.
...
To add insult to injury, I whipped up a hacky brotli cache warmup bash script that runs curl after cache busting.
Now most of the time no real visitor has to wait.

With all that in place, when I'm physically near the Pi, this post loads XXX kilobytes of brotli-compressed HTML in YYYms.
