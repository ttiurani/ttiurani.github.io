...
Because of my non-standard needs I had to write https://codeberg.org/convivial.software/peermerge/src/branch/main/peermerge-server/src/http/html.rs[my own static file serving] to be able to serve Svelte files, return the appropriate headers and inline CSS for blog posts.
...
First, I do https://codeberg.org/convivial.software/peermerge/src/branch/main/peermerge-server/src/logging.rs[async access logging in a low priority thread] into UTC date formatted log files, which is also good for performance.
...
From this access log, I then https://codeberg.org/convivial.software/peermerge/src/branch/main/peermerge-server/src/metrics.rs[update every minute a UTC daily metric file].
...
To make this happen, I wrote https://codeberg.org/convivial.software/peermerge/src/branch/main/peermerge-hub/src/backup.rs[a backup process] that creates a `tar.gz` file from the metrics files.
...
For this reason, I changed the file system cache to a https://codeberg.org/convivial.software/peermerge/src/branch/main/peermerge-server/src/http/cache.rs[cache middleware for axum], so that I cache the entire compressed HTTP response.
...
That's why I now cache permanently and implemented a https://codeberg.org/convivial.software/peermerge/src/branch/main/peermerge-hub/src/admin.rs[listener for a unix socket that does cache busting on demand], which I call with Ansible when the site updates.
